//! Minimal Language‑Server‑Protocol example: **`minimal_asp.rs`**
//! =============================================================
//!
//! | ↔ / ← | ASP method | What the implementation does |
//! |-------|------------|------------------------------|
...
//! | ← | `textDocument/completion` | offers one hard‑coded item `HelloFromASP` |
//! | ← | `textDocument/hover` | shows *Hello from minimal_asp* markdown |
//! | ← | `textDocument/formatting` | pipes the doc through **rustfmt** and returns a full‑file edit |
//!
//! ### Quick start
//! ```bash
//! cd rust-analyzer/lib/asp-server
//! cargo run --example minimal_asp
//! ```
//!
...
//!
//! # 4. completion - expect HelloFromASP
//! Content-Length: 139
    ...
    for msg in &connection.receiver {
        ...
        match msg {
            Message::Request(req) => {
                ...
                //                     log::error!("[asp] request {} failed: {err}", &req.method);
                //                 }
            }
            //             Message::Notification(note) => {
            //                 if let Err(err) = handle_notification(&connection, &note, &mut docs) {
            //                     log::error!("[asp] notification {} failed: {err}", note.method);
            //                 }
            //             }
            //             Message::Response(resp) => log::error!("[asp] response: {resp:?}"),
            _ => {}
        }
    }
    Ok(())
...
//             send_ok(conn, req.id.clone(), &asp_types::GotoDefinitionResponse::Array(Vec::new()))?;
//         }
//         Completion::METHOD => {
//             let item = CompletionItem {
//                 label: "HelloFromASP".into(),
...
//             conn,
//             req.id.clone(),
//             asp_server::ErrorCode::MethodNotFound,
//             "unhandled method",
//         )?,
//     }
//     Ok(())
...
//     conn.sender.send(Message::Notification(asp_server::Notification::new(
...
//     conn: &Connection,
//     id: RequestId,
//     code: asp_server::ErrorCode,
//     msg: &str,
// ) -> Result<()> {
//     let resp = Response {
//         id,
//         result: None,
//         error: Some(asp_server::ResponseError {
//             code: code as i32,
//             message: msg.into(),
//             data: None,
//         }),
